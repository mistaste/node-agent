...
func TestMarkActiveUsersCountsOnlyRecentTrafficGrowth(t *testing.T) {
	...
	if got := c.markActiveUsers(now, []xray.UserTraffic{{UUID: "u1", Uplink: 100, Downlink: 0}}, nil); len(got) != 0 {
		t.Fatalf("first cumulative sample active users = %d, want 0", len(got))
	}
	if got := c.markActiveUsers(now.Add(15*time.Second), []xray.UserTraffic{{UUID: "u1", Uplink: 120, Downlink: 0}}, nil); len(got) != 1 || got[0].UUID != "u1" {
		t.Fatalf("traffic growth active users = %+v, want u1", got)
	}
	if got := c.markActiveUsers(now.Add(60*time.Second), []xray.UserTraffic{{UUID: "u1", Uplink: 120, Downlink: 0}}, nil); len(got) != 1 {
		t.Fatalf("recent idle active users = %d, want 1", len(got))
	}
	if got := c.markActiveUsers(now.Add(120*time.Second), []xray.UserTraffic{{UUID: "u1", Uplink: 120, Downlink: 0}}, nil); len(got) != 0 {
